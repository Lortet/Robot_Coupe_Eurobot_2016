...
class Moteurs{
  ...
  private:
    static const boolean PWM_REVERSE = true;            // Inversion des PWM (false => PWM 0 = arret & PWM max = vitesse max; true => PWM 0 = vitesse max & PWM max = arret)
      ...
      static const int MAX_PWM_L = 64;                    // PWM maximale correspondant à la vitesse maximale de la roue gauche (A determiner à l'aide du programme DEBUG_Moteurs_AutoCoeffs)
      
      static const int MIN_PWM_R = 16;                    // PWM minimale permettant à la roue droite de bouger (A determiner à l'aide du programme DEBUG_Moteurs_AutoCoeffs)
      static const int MAX_PWM_R = 60;                    // PWM maximale correspondant à la vitesse maximale de la roue droite (A determiner à l'aide du programme DEBUG_Moteurs_AutoCoeffs)
