...
bool NewCAN::isInitCan0var;
bool NewCAN::isInitCan1var;

CAN_FRAME NewCAN::can0Data[MAX_CAN_ID];
long NewCAN::can0Time[MAX_CAN_ID];
    
CAN_FRAME NewCAN::can1Data[MAX_CAN_ID];
long NewCAN::can1Time[MAX_CAN_ID];
...
void NewCAN::step() {
  ...
  if( isInitCan0() ){
    ...
    startTime = micros();
    int newMsg = Can0.available();
    while( micros()-startTime < maxTime && newMsg ) {
      Serial.print("Messages disponibles (" + String( newMsg ) + ")");
      
      if(!Can0.read(frame)) Serial.println("erreur lecture CAN");
      Serial.println( " : " + String( frame.id ) );
      if( frame.id < MAX_CAN_ID ) { // < 0x400
        can0Time[ frame.id ] = micros();
        can0Data[ frame.id ] = frame;
      }
      newMsg = Can0.available();
    }
  }
