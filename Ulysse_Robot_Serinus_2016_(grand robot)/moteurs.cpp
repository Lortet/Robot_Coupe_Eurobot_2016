...
void Moteurs::set( pin moteur, long consigne ) {
  ...
  if( consigne != 0 ) consigne = (long)( (float)abs( consigne )/255 * ( MAX_PWM-MIN_PWM ) * moteursSpeed + MIN_PWM );
  if( (long)( (float)consigne > MAX_PWM * moteursSpeed ) ) consigne = (int)( (float)MAX_PWM * moteursSpeed );
...
void Moteurs::setSpeed( float speed ) {
  moteursSpeed = speed;
}
